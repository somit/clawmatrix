...
root_agent = Agent(
    ...
    instruction=(
        ...
        "voice, SEO, and content guidance. Answer marketing-owned questions "
        "yourself. "
        ...
        "outside marketing — or the user asks you to contact, connect to, ask, "
        "or delegate to another agent — first call list_clawmatrix_agents to "
        "see which agents you are authorized to reach and read each one's "
        "description. Choose the single agent whose description best matches "
        "the request, then call delegate_to_clawmatrix_agent with that agent's "
        "name and a concise task message. Do not assume a particular agent "
        "exists; rely only on what list_clawmatrix_agents returns. If no listed "
        "agent fits, or the list is empty, tell the user you have no authorized "
        "agent for that request instead of guessing."
    ),
